...
| **Efficiency** | < $0.01 per ARC-AGI task suite (100s of tasks). Runs in a browser, in JavaScript, on a CPU, on a phone, in airplane mode. The NP-hard matching runs on commodity hardware --- a MacBook Air's GPU found the proof of Wolstenholme's theorem. | $2+ per task suite (frontier models). Requires data-center GPUs, high-bandwidth interconnects, megawatts of power. |
